...
思维链可视化模块是 NecoRAG 框架中一个创新性的可解释性组件，旨在将复杂的 AI 推理过程转化为用户可理解的可视化文本。该模块的核心价值在于提供透明的决策过程展示，帮助用户理解 AI 如何从输入查询到最终答案的完整思考路径。

该模块的设计理念基于"让 AI 思考过程可见化"的原则，通过三个关键维度来展示推理过程：
- **检索路径**：展示从查询理解到最终答案的完整信息检索流程
- **证据来源**：明确标注每个断言的证据支持和相关度
- **推理过程**：呈现多跳推理的逻辑链条和决策依据

## 项目结构

思维链可视化模块位于 NecoRAG 框架的响应层，与记忆管理、检索、精炼等核心模块紧密集成。整个系统采用分层架构设计，确保各模块职责清晰且耦合度适中。
