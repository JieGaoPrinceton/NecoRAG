...
NecoRAG 是一个创新的认知型 RAG 框架，模拟人脑的双系统记忆理论和猫科动物的敏捷直觉。通过五层架构设计，实现了从感知到交互的完整认知闭环。

### 核心特性

- 🧠 **类脑记忆结构**：三层记忆系统（工作记忆 L1 + 语义记忆 L2 + 情景图谱 L3）
- 🐱 **猫式敏捷反应**：毫秒级响应，Pounce 机制精准捕捉关键信息
- 🔄 **自我反思能力**：Grooming Agent 幻觉自检与知识进化
- 🎨 **可解释性输出**：思维链可视化，展示推理过程
- ⚙️ **配置管理系统**：Web Dashboard 实时配置和监控

## 🏗️ 核心架构

NecoRAG 采用"五感一心"分层架构，每一层对应猫的生物特性与人脑认知机制：
...
# 1. 初始化组件
engine = WhiskersEngine(chunk_size=512)
...
📖 [详细文档](necorag/whiskers/README.md)

---

### 2. Nine-Lives Memory - 九命记忆存储
...
📖 [详细文档](necorag/memory/README.md)

---

### 3. Pounce Strategy - 扑击检索策略
...
- ⚡ Pounce 机制（一旦置信度超过阈值，立即终止检索）

**使用示例**：
```python
from necorag import PounceRetriever
...
📖 [详细文档](necorag/retrieval/README.md)

---

### 4. Grooming Agent - 梳理校正代理
...
**使用示例**：
```python
from necorag import GroomingAgent
...
📖 [详细文档](necorag/grooming/README.md)

---

### 5. Purr Interface - 呼噜交互接口
...
**使用示例**：
```python
from necorag import PurrInterface
...
- 🔄 集成 LangGraph 实现 Grooming Agent
- 🔄 动态重排序与 Novelty 检测
...
### 模块文档
- [Whiskers Engine](necorag/whiskers/README.md) - 感知层设计
- [Nine-Lives Memory](necorag/memory/README.md) - 记忆层设计
- [Pounce Strategy](necorag/retrieval/README.md) - 检索层设计
- [Grooming Agent](necorag/grooming/README.md) - 巩固层设计
- [Purr Interface](necorag/purr/README.md) - 交互层设计
- [Dashboard](necorag/dashboard/README.md) - 配置管理系统
...
NecoRAG is an innovative cognitive RAG framework that simulates the human brain's dual-system memory theory and feline agility. Through a five-layer architecture design, it achieves a complete cognitive loop from perception to interaction.

### Key Features

- 🧠 **Brain-like Memory Structure**: Three-layer memory system (L1 Working + L2 Semantic + L3 Episodic)
- 🐱 **Cat-like Agility**: Millisecond response with Pounce mechanism
- 🔄 **Self-reflection**: Grooming Agent for hallucination detection and knowledge evolution
...
NecoRAG adopts a "five senses and one heart" layered architecture:

1. **Whiskers Engine** (Perception) - Document parsing and encoding
2. **Nine-Lives Memory** (Memory) - Three-layer storage system
3. **Pounce Strategy** (Retrieval) - Hybrid search and re-ranking
4. **Grooming Agent** (Consolidation) - Hallucination detection and knowledge consolidation
5. **Purr Interface** (Interaction) - Context-adaptive generation
...
| Hallucination Rate | < 5% | Via Grooming Agent |
