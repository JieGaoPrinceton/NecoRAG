...
subgraph "核心依赖"
A[numpy>=3.3.0-alpha]
B[python-dateutil>=3.3.0-alpha]
end
subgraph "Dashboard依赖"
C[fastapi>=3.3.0-alpha]
D[uvicorn>=3.3.0-alpha]
E[pydantic>=3.3.0-alpha]
end
subgraph "文档解析"
F[ragflow]
G[PyMuPDF]
...
- transformers>=3.3.0-alpha（深度学习模型）
- torch>=3.3.0-alpha（PyTorch）

**任务调度依赖**：
- apscheduler>=3.3.0-alpha（定时任务）
- celery>=3.3.0-alpha（分布式任务队列）
- redis>=3.3.0-alpha（Celery broker）
