...
| 模块 | 新增依赖 | 用途 |
|------|---------|------|
| **Intent Analysis** | `jieba>=0.42.1` | 中文分词与关键词提取 |
| **Domain Weight** | `scipy>=1.11.0` | 科学计算，相似度计算 |
| **Monitoring** | `prometheus-client>=0.19.0` | Prometheus 指标收集 |
| **Security** | `PyJWT>=2.8.0` | JWT 认证 |
| **Security** | `python-jose[cryptography]>=3.3.0` | OAuth2 支持 |
| **A/B Testing** | `statsmodels>=0.14.0` | 统计分析模型 |
| **Visualization** | `plotly>=5.18.0` | 交互式图表 |
| **Visualization** | `matplotlib>=3.8.0` | 静态图表 |
| **Adaptive Opt.** | `scikit-learn>=1.3.0` | 机器学习，偏好预测 |
| **Dashboard** | `websockets>=12.0` | WebSocket 实时通信 |
| **Testing** | `pytest-cov>=4.1.0` | 测试覆盖率统计 |
