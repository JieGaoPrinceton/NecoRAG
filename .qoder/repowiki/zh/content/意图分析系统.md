...
## 项目结构
意图分析系统位于 `src/intent/` 目录下，同时与智能路由引擎紧密集成：
...
subgraph "集成层"
O[necorag.py] --> I
P[retrieval/models.py] --> Q[RetrievalResult]
R[response/models.py] --> S[Response]
end
subgraph "配置层"
T[core/config.py] --> U[NecoRAGConfig]
V[IntentConfig] --> W[IntentRoutingStrategy]
end
B --> E
C --> B
C --> D
...
subgraph "集成层"
O[NecoRAG] --> G
P[AdaptiveRetriever] --> E
Q[ResponseInterface] --> D
end
subgraph "配置层"
R[NecoRAGConfig] --> A
S[RetrievalConfig] --> P
...
## 结论
意图分析系统通过精心设计的多层架构，提供了高效、准确的查询意图识别能力。随着与智能路由引擎的深度集成，系统不仅能够进行基础的意图分类，还能实现自适应学习和优化，为用户提供更加个性化的检索体验。
