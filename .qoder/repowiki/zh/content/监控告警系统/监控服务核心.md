...
| 依赖库 | 版本要求 | 用途 | 重要性 |
|-------|---------|------|--------|
| FastAPI | >=0.109.0 | Web 框架 | 核心依赖 |
| APScheduler | >=3.10.0 | 异步调度器 | 核心依赖 |
| psutil | >=5.0.0 | 系统信息收集 | 核心依赖 |
| aiohttp | >=3.9.0 | 异步 HTTP 客户端 | 核心依赖 |
| pydantic | >=2.5.0 | 数据验证 | 核心依赖 |
| prometheus-client | >=0.19.0 | 指标导出 | 可选依赖 |
