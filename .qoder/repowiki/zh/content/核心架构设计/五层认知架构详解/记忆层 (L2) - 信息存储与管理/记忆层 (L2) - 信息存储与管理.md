# 记忆层 (L2) - 信息存储与管理

<cite>
**本文引用的文件**
- [src/memory/__init__.py](file://src/memory/__init__.py)
- [src/memory/manager.py](file://src/memory/manager.py)
- [src/memory/models.py](file://src/memory/models.py)
- [src/memory/working_memory.py](file://src/memory/working_memory.py)
- [src/memory/semantic_memory.py](file://src/memory/semantic_memory.py)
- [src/memory/episodic_graph.py](file://src/memory/episodic_graph.py)
- [src/memory/decay.py](file://src/memory/decay.py)
- [src/memory/backends/base.py](file://src/memory/backends/base.py)
- [src/perception/models.py](file://src/perception/models.py)
- [src/core/protocols.py](file://src/core/protocols.py)
- [wiki/wiki/记忆管理层/三层记忆架构.md](file://wiki/wiki/记忆管理层/三层记忆架构.md)
- [wiki/wiki/记忆管理层/记忆衰减与巩固机制.md](file://wiki/wiki/记忆管理层/记忆衰减与巩固机制.md)
- [wiki/wiki/记忆管理层/记忆存储后端.md](file://wiki/wiki/记忆管理层/记忆存储后端.md)
- [tests/test_memory/test_working_memory.py](file://tests/test_memory/test_working_memory.py)
- [tests/test_memory/test_decay.py](file://tests/test_memory/test_decay.py)
</cite>

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件聚焦 NecoRAG 记忆层（L2）的语义记忆系统，系统阐述其如何模拟人类大脑的三层记忆结构：工作记忆（L1）、语义记忆（L2）与情景图谱（L3）。文档详细说明记忆管理器如何协调三层记忆的存储与检索，语义记忆如何承载长期知识与向量检索，以及记忆衰减机制如何模拟生物记忆的巩固与遗忘过程。同时提供接口契约、数据流转、内存管理与性能优化的最佳实践，帮助开发者在理解现有实现的基础上进行扩展与生产化部署。

## 项目结构
记忆层位于 src/memory 目录，包含三层记忆实现、统一管理器、衰减机制以及可插拔的存储后端抽象与内存实现。感知层提供编码后的文本块（含稠密/稀疏向量、实体三元组等），作为记忆层输入。

```mermaid
graph TB
subgraph "记忆层"
MM["MemoryManager<br/>统一管理器"]
WM["WorkingMemory<br/>L1 工作记忆"]
SM["SemanticMemory<br/>L2 语义记忆"]
EG["EpisodicGraph<br/>L3 情景图谱"]
DEC["MemoryDecay<br/>记忆衰减"]
end
subgraph "感知层"
ENC["EncodedChunk<br/>编码文本块"]
end
subgraph "存储后端"
BVS["BaseVectorStore<br/>向量存储抽象"]
BGS["BaseGraphStore<br/>图存储抽象"]
IMS["InMemoryVectorStore<br/>内存向量存储"]
IMG["InMemoryGraphStore<br/>内存图存储"]
end
ENC --> MM
MM --> WM
MM --> SM
MM --> EG
MM --> DEC
SM --> BVS
EG --> BGS
BVS --> IMS
BGS --> IMG
```

图表来源
- [src/memory/manager.py:16-195](file://src/memory/manager.py#L16-L195)
- [src/memory/working_memory.py:11-120](file://src/memory/working_memory.py#L11-L120)
- [src/memory/semantic_memory.py:21-179](file://src/memory/semantic_memory.py#L21-L179)
- [src/memory/episodic_graph.py:10-194](file://src/memory/episodic_graph.py#L10-L194)
- [src/memory/decay.py:11-155](file://src/memory/decay.py#L11-L155)
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)
- [src/perception/models.py:31-41](file://src/perception/models.py#L31-L41)

章节来源
- [src/memory/__init__.py:1-29](file://src/memory/__init__.py#L1-L29)
- [src/memory/manager.py:16-195](file://src/memory/manager.py#L16-L195)
- [src/memory/working_memory.py:11-120](file://src/memory/working_memory.py#L11-L120)
- [src/memory/semantic_memory.py:21-179](file://src/memory/semantic_memory.py#L21-L179)
- [src/memory/episodic_graph.py:10-194](file://src/memory/episodic_graph.py#L10-L194)
- [src/memory/decay.py:11-155](file://src/memory/decay.py#L11-L155)
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)
- [src/perception/models.py:31-41](file://src/perception/models.py#L31-L41)

## 核心组件
- MemoryManager：统一协调 L1/L2/L3 三层记忆，负责存储、检索、巩固与主动遗忘。
- WorkingMemory（L1）：会话上下文与意图轨迹的短期存储，具备 TTL 过期与 LRU 淘汰能力。
- SemanticMemory（L2）：高维向量存储与检索，支持混合检索与元数据更新。
- EpisodicGraph（L3）：实体关系网络，支持多跳查询与因果链条追踪。
- MemoryDecay：基于指数衰减与访问频率的动态权重机制，实现记忆巩固与归档。
- 存储后端抽象：BaseVectorStore/BaseGraphStore；内存实现 InMemoryVectorStore/InMemoryGraphStore。

章节来源
- [src/memory/manager.py:16-195](file://src/memory/manager.py#L16-L195)
- [src/memory/working_memory.py:11-120](file://src/memory/working_memory.py#L11-L120)
- [src/memory/semantic_memory.py:21-179](file://src/memory/semantic_memory.py#L21-L179)
- [src/memory/episodic_graph.py:10-194](file://src/memory/episodic_graph.py#L10-L194)
- [src/memory/decay.py:11-155](file://src/memory/decay.py#L11-L155)
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)

## 架构总览
三层记忆的协作流程如下：
- 新知识入库：感知层编码后，MemoryManager 将向量写入 L2，实体三元组写入 L3，同时维护统一内存索引。
- 检索流程：先从 L1 获取会话上下文与意图轨迹，再进行 L2 向量检索，必要时结合 L3 图谱推理，最后融合结果。
- 记忆巩固：周期性应用衰减，强化高频访问记忆，归档低权重记忆。

```mermaid
sequenceDiagram
participant Per as "感知层"
participant MM as "MemoryManager"
participant WM as "WorkingMemory(L1)"
participant SM as "SemanticMemory(L2)"
participant EG as "EpisodicGraph(L3)"
Per->>MM : "EncodedChunk"
MM->>SM : "store_vectors()"
MM->>EG : "add_entity/add_relation()"
MM->>MM : "维护统一内存索引"
Note over MM : "检索阶段"
MM->>WM : "获取会话上下文/意图轨迹"
MM->>SM : "search()/hybrid_search()"
SM-->>MM : "向量检索结果"
MM->>EG : "multi_hop_query()/find_causal_chain()"
EG-->>MM : "图谱推理结果"
MM-->>Per : "融合后的检索结果"
```

图表来源
- [src/memory/manager.py:48-147](file://src/memory/manager.py#L48-L147)
- [src/memory/semantic_memory.py:50-142](file://src/memory/semantic_memory.py#L50-L142)
- [src/memory/episodic_graph.py:71-147](file://src/memory/episodic_graph.py#L71-L147)
- [src/memory/working_memory.py:36-85](file://src/memory/working_memory.py#L36-L85)

## 详细组件分析

### L1 工作记忆（WorkingMemory）
- 功能定位：短期会话上下文与用户意图轨迹存储，模拟“瞬时遗忘”，适合低延迟、易失性数据。
- 数据存储：内存字典模拟 Redis，支持会话级上下文合并、意图列表追加。
- 检索机制：按会话 ID 获取上下文与意图轨迹；TTL 过期与 LRU 淘汰为 TODO，当前最小实现返回占位行为。
- 适用场景：多轮对话、意图追踪、临时状态缓存。
- 配置要点：TTL、单会话最大条目数、LRU 最大缓存数（见配置表）。

```mermaid
classDiagram
class WorkingMemory {
+int ttl
+int max_session_items
+add_context(session_id, context) void
+get_context(session_id) Dict
+track_intent(session_id, intent) void
+get_intent_trajectory(session_id) List
+clear_session(session_id) void
+clear_expired() int
+exists(session_id) bool
}
```

图表来源
- [src/memory/working_memory.py:11-120](file://src/memory/working_memory.py#L11-L120)

章节来源
- [src/memory/working_memory.py:11-120](file://src/memory/working_memory.py#L11-L120)

### L2 语义记忆（SemanticMemory）
- 功能定位：高维向量存储与模糊匹配，支持向量检索与混合检索（关键词+向量）。
- 数据存储：内存字典模拟向量库，存储向量与元数据；支持元数据更新与删除。
- 检索机制：余弦相似度计算；HNSW 索引为 TODO；混合检索为 TODO。
- 适用场景：语义近似检索、模糊匹配、直觉检索。
- 配置要点：集合名、向量维度、索引类型（见配置表）。

```mermaid
classDiagram
class SemanticMemory {
+str collection_name
+int vector_size
+store_vectors(items) str[]
+search(query_vector, top_k, min_score) SearchResult[]
+hybrid_search(query_vector, keywords, top_k, vector_weight) SearchResult[]
+update_metadata(memory_id, metadata) bool
+delete(memory_id) bool
}
class SearchResult {
+str memory_id
+str content
+float score
+Dict metadata
}
SemanticMemory --> SearchResult : "返回"
```

图表来源
- [src/memory/semantic_memory.py:21-179](file://src/memory/semantic_memory.py#L21-L179)

章节来源
- [src/memory/semantic_memory.py:21-179](file://src/memory/semantic_memory.py#L21-L179)

### L3 情景图谱（EpisodicGraph）
- 功能定位：实体关系网络，支持多跳推理与因果链条追踪，模拟“结构化记忆”。
- 数据存储：内存图结构，存储实体与关系；支持邻接表与遍历。
- 检索机制：BFS 多跳查询、因果关系链查找、相关实体获取。
- 适用场景：复杂推理、事件链条、知识图谱路径发现。
- 配置要点：最大关系深度、是否启用因果图谱（见配置表）。

```mermaid
classDiagram
class EpisodicGraph {
+int max_relation_depth
+add_entity(entity) str
+add_relation(source, target, relation) str
+multi_hop_query(start, hops, relation_types) GraphPath[]
+find_causal_chain(event) Relation[]
+get_related_entities(entity_id, depth) Entity[]
+get_entity(entity_id) Entity
}
class Entity {
+str entity_id
+str name
+str entity_type
+Dict properties
}
class Relation {
+str source_id
+str target_id
+str relation_type
+float strength
+Dict properties
}
class GraphPath {
+str[] nodes
+Relation[] edges
+float total_strength
}
EpisodicGraph --> Entity : "管理"
EpisodicGraph --> Relation : "管理"
EpisodicGraph --> GraphPath : "返回"
```

图表来源
- [src/memory/episodic_graph.py:10-194](file://src/memory/episodic_graph.py#L10-L194)
- [src/memory/models.py:33-67](file://src/memory/models.py#L33-L67)

章节来源
- [src/memory/episodic_graph.py:10-194](file://src/memory/episodic_graph.py#L10-L194)
- [src/memory/models.py:33-67](file://src/memory/models.py#L33-L67)

### 记忆管理器（MemoryManager）
- 统一入口：负责将感知层编码后的知识写入 L2/L3，并维护统一内存索引。
- 检索控制：根据层级选择 L2/L3 检索，强化访问记忆权重。
- 巩固与遗忘：应用衰减、归档低权重记忆、删除统一索引。

```mermaid
flowchart TD
Start(["开始"]) --> Encode["感知层编码<br/>EncodedChunk"]
Encode --> StoreL2["写入 L2 向量库"]
Encode --> StoreL3["写入 L3 图谱"]
StoreL2 --> Index["维护统一索引"]
StoreL3 --> Index
Index --> Retrieve["检索请求"]
Retrieve --> L2Search["L2 向量检索"]
L2Search --> Reinforce["强化访问记忆权重"]
Reinforce --> L3Query["L3 图谱推理(可选)"]
L3Query --> Fuse["融合结果"]
Fuse --> End(["结束"])
```

图表来源
- [src/memory/manager.py:48-147](file://src/memory/manager.py#L48-L147)

章节来源
- [src/memory/manager.py:16-195](file://src/memory/manager.py#L16-L195)

### 记忆衰减（MemoryDecay）
- 设计原理：指数时间衰减 × 访问频率因子，实现“巩固与遗忘”的动态平衡。
- 功能：计算权重、批量衰减、归档低权重记忆、强化访问记忆。

```mermaid
flowchart TD
S(["开始"]) --> Calc["计算权重<br/>weight(t)=initial_weight×e^(-λt)×access_frequency"]
Calc --> Apply["批量应用衰减"]
Apply --> Archive{"是否低于阈值?"}
Archive --> |是| Move["归档/删除"]
Archive --> |否| Keep["保留"]
Move --> E(["结束"])
Keep --> E
```

图表来源
- [src/memory/decay.py:39-155](file://src/memory/decay.py#L39-L155)

章节来源
- [src/memory/decay.py:11-155](file://src/memory/decay.py#L11-L155)

### 存储后端抽象与内存实现
- 抽象接口：BaseVectorStore/ BaseGraphStore 定义统一的向量与图存储接口。
- 内存实现：InMemoryVectorStore/ InMemoryGraphStore 提供开发与测试可用的最小实现，便于替换为真实外部数据库（如 Qdrant、Neo4j 等）。

```mermaid
classDiagram
class BaseVectorStore {
<<abstract>>
+upsert(records) str[]
+search(query_vector, top_k, filters, threshold) SearchResult[]
+get(ids) Optional[]VectorRecord~~
+delete(ids) int
+count() int
+dimension() int
}
class BaseGraphStore {
<<abstract>>
+add_node(node) str
+add_edge(edge) str
+get_node(node_id) Optional~GraphNode~
+get_neighbors(node_id, edge_types, direction) Tuple[]
+traverse(start_id, max_depth, edge_types) GraphNode[]
+find_path(source_id, target_id, max_depth) Optional~GraphNode[]~
+delete_node(node_id) bool
+delete_edge(edge_id) bool
+node_count() int
+edge_count() int
}
class InMemoryVectorStore
class InMemoryGraphStore
BaseVectorStore <|-- InMemoryVectorStore
BaseGraphStore <|-- InMemoryGraphStore
```

图表来源
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)

章节来源
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)

## 依赖关系分析
- MemoryManager 依赖 WorkingMemory、SemanticMemory、EpisodicGraph 与 MemoryDecay。
- SemanticMemory 与 EpisodicGraph 通过抽象接口 BaseVectorStore/BaseGraphStore 与具体实现解耦。
- 感知层 EncodedChunk 为 MemoryManager 的输入，包含稠密向量、稀疏向量、实体三元组与情境标签。

```mermaid
graph LR
ENC["EncodedChunk"] --> MM["MemoryManager"]
MM --> WM["WorkingMemory"]
MM --> SM["SemanticMemory"]
MM --> EG["EpisodicGraph"]
MM --> DEC["MemoryDecay"]
SM --> BVS["BaseVectorStore"]
EG --> BGS["BaseGraphStore"]
BVS --> IMS["InMemoryVectorStore"]
BGS --> IMG["InMemoryGraphStore"]
```

图表来源
- [src/memory/manager.py:48-112](file://src/memory/manager.py#L48-L112)
- [src/memory/backends/base.py:54-297](file://src/memory/backends/base.py#L54-L297)
- [src/perception/models.py:31-41](file://src/perception/models.py#L31-L41)

章节来源
- [src/memory/manager.py:16-195](file://src/memory/manager.py#L16-L195)
- [src/perception/models.py:31-41](file://src/perception/models.py#L31-L41)

## 性能考量
- 写入延迟：L1 < L2 < L3（参考性能指标表）。
- 检索延迟：L1 < L2 < L3（参考性能指标表）。
- 容量规模：L1 适合小规模短期数据；L2 支持千万级向量；L3 支持亿级节点。
- 优化建议：在 L2 引入 HNSW 索引与混合检索；在 L3 实现图遍历剪枝与缓存热点路径；在 L1 引入 TTL 过期检测与 LRU 淘汰。

章节来源
- [wiki/wiki/记忆管理层/三层记忆架构.md:360-367](file://wiki/wiki/记忆管理层/三层记忆架构.md#L360-L367)

## 故障排查指南
- L1 会话上下文缺失：检查 add_context 与 get_context 的 session_id 是否一致；确认 clear_session 是否被误触发。
- L2 检索为空：确认向量维度与集合名配置正确；检查 query 向量维度与存储向量维度一致；验证 min_score 与 top_k 设置。
- L3 查询无结果：确认实体 ID 与关系类型过滤；检查 max_relation_depth 与 relation_types；验证图谱是否已正确写入。
- 记忆权重异常：检查 decay_rate 与 archive_threshold；确认 reinforce 是否被频繁调用导致权重上限。

章节来源
- [src/memory/working_memory.py:36-120](file://src/memory/working_memory.py#L36-L120)
- [src/memory/semantic_memory.py:50-179](file://src/memory/semantic_memory.py#L50-L179)
- [src/memory/episodic_graph.py:71-194](file://src/memory/episodic_graph.py#L71-L194)
- [src/memory/decay.py:72-155](file://src/memory/decay.py#L72-L155)

## 结论
三层记忆架构以“工作记忆 + 语义记忆 + 情景图谱”的组合，模拟人类记忆的多层结构与动态平衡。通过 MemoryManager 统一编排、MemoryDecay 实现巩固与遗忘、抽象存储后端实现可插拔扩展，该系统既满足快速开发与测试，又为生产环境预留了高性能向量与图数据库的接入空间。

## 附录

### 数据模型说明
- MemoryItem：记忆项，包含内容、层级、向量、元数据、权重、访问计数与时间戳。
- Entity/Relation/GraphPath：实体、关系与图谱路径，用于 L3 图谱。
- EncodedChunk：感知层编码后的文本块，包含稠密/稀疏向量、实体三元组与情境标签。

```mermaid
erDiagram
MEMORY_ITEM {
string memory_id PK
string content
enum layer
array vector
json metadata
float weight
int access_count
timestamp created_at
timestamp last_accessed
}
ENTITY {
string entity_id PK
string name
string entity_type
json properties
}
RELATION {
string source_id FK
string target_id FK
string relation_type
float strength
json properties
}
GRAPH_PATH {
string nodes
string edges
float total_strength
}
MEMORY_ITEM ||--o{ ENTITY : "被实体关联"
ENTITY ||--o{ RELATION : "作为起点/终点"
RELATION ||--o{ ENTITY : "连接"
GRAPH_PATH }o--|| RELATION : "包含"
```

图表来源
- [src/memory/models.py:19-67](file://src/memory/models.py#L19-L67)

章节来源
- [src/memory/models.py:12-67](file://src/memory/models.py#L12-L67)
- [src/perception/models.py:31-41](file://src/perception/models.py#L31-L41)

### 接口契约与数据流转
- 存储契约：MemoryManager.store 接收 EncodedChunk，写入 L2 向量库与 L3 图谱，并维护统一内存索引。
- 检索契约：MemoryManager.retrieve 支持指定层级检索，默认启用 L2 向量检索；命中后对记忆进行强化。
- 巩固契约：MemoryManager.consolidate 与 forget 提供周期性巩固与主动遗忘策略，基于 MemoryDecay 的权重计算与归档阈值。

章节来源
- [src/memory/manager.py:48-147](file://src/memory/manager.py#L48-L147)
- [src/memory/decay.py:72-155](file://src/memory/decay.py#L72-L155)

### 内存管理与性能优化最佳实践
- 内存实现现状：当前以内存字典模拟存储，适合开发与测试；生产环境建议替换为 Redis、Qdrant、Neo4j 等真实存储。
- 并发控制：内存实现未引入锁或并发控制，建议在多线程/多进程环境中增加读写锁或使用线程安全容器。
- 资源清理：定期清理过期会话与低价值记忆，控制内存占用；合理设置 TTL 与会话上限。
- 扩展建议：在 L2 引入 HNSW 索引与混合检索；在 L3 实现图遍历剪枝与缓存热点路径；在 L1 引入 TTL 过期检测与 LRU 淘汰。

章节来源
- [wiki/wiki/记忆管理层/记忆存储后端.md:311-322](file://wiki/wiki/记忆管理层/记忆存储后端.md#L311-L322)
- [wiki/wiki/记忆管理层/记忆存储后端.md:324-340](file://wiki/wiki/记忆管理层/记忆存储后端.md#L324-L340)

### 测试用例参考
- 工作记忆测试：覆盖上下文操作、意图轨迹跟踪、会话管理与多会话隔离等场景。
- 记忆衰减测试：覆盖权重计算、批量衰减、归档判断、记忆强化与边界情况。

章节来源
- [tests/test_memory/test_working_memory.py:1-307](file://tests/test_memory/test_working_memory.py#L1-L307)
- [tests/test_memory/test_decay.py:1-544](file://tests/test_memory/test_decay.py#L1-L544)