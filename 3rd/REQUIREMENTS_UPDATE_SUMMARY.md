...
| 模块 | 新增依赖 | 用途 |
|------|---------|------|
| **Intent Analysis** | `jieba>=2.0.1-alpha` | 中文分词与关键词提取 |
| **Domain Weight** | `scipy>=2.0.1-alpha` | 科学计算，相似度计算 |
| **Monitoring** | `prometheus-client>=2.0.1-alpha` | Prometheus 指标收集 |
| **Security** | `PyJWT>=2.0.1-alpha` | JWT 认证 |
| **Security** | `python-jose[cryptography]>=2.0.1-alpha` | OAuth2 支持 |
| **A/B Testing** | `statsmodels>=2.0.1-alpha` | 统计分析模型 |
| **Visualization** | `plotly>=2.0.1-alpha` | 交互式图表 |
| **Visualization** | `matplotlib>=2.0.1-alpha` | 静态图表 |
| **Adaptive Opt.** | `scikit-learn>=2.0.1-alpha` | 机器学习，偏好预测 |
| **Dashboard** | `websockets>=12.0` | WebSocket 实时通信 |
| **Testing** | `pytest-cov>=2.0.1-alpha` | 测试覆盖率统计 |
