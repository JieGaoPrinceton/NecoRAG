...
## 简介
精炼代理（Refinement Agent）负责在答案生成后进行"生成-批判-修正-幻觉检测"闭环验证，并结合知识固化与记忆修剪，确保输出的可靠性与长期稳定性。该系统是对原有Grooming模块的重构升级，现已重命名为Refinement系统，提供更完善的答案质量保证机制。
...
该层与检索层（提供证据）和交互层（生成最终可读响应）紧密协作，形成"检索-精炼-交互"的完整闭环。

## 项目结构
精炼系统位于 src/refinement 目录，包含以下关键模块：
...
```mermaid
graph TB
subgraph "精炼系统(src/refinement)"
A["agent.py<br/>RefinementAgent"]
...
- [src/refinement/generator.py:9-64](file://src/refinement/generator.py#L9-L64)
...
## 核心组件
- RefinementAgent：主控制器，封装生成、批判、修正、幻觉检测、知识固化与修剪的全流程；支持最大迭代次数与最低置信度阈值控制。
...
## 架构总览
精炼系统与记忆层的关系体现在：当初始化时传入 MemoryManager，RefinementAgent 将启用 KnowledgeConsolidator 与 MemoryPruner；否则仅执行在线验证流程。记忆层提供统一存储与检索能力，支持向量检索与图谱实体关系。
...
### RefinementAgent：生成-批判-修正循环与幻觉检测
...
GA-->>U : "RefinementResult(最终)"
```

**图表来源**
- [src/refinement/agent.py:61-128](file://src/refinement/agent.py#L61-L128)
- [src/refinement/generator.py:25-63](file://src/refinement/generator.py#L25-L63)
...
- RefinementResult：查询、答案、置信度、引用、幻觉报告、迭代次数、元数据
...
```mermaid
graph LR
GA["RefinementAgent"] --> GEN["Generator"]
GA --> CRIT["Critic"]
GA --> REF["Refiner"]
...
- 生成答案为空
  ...
  - 参考：[src/refinement/generator.py:45-50](file://src/refinement/generator.py#L45-L50)
- 答案未通过批判
  - 查看 Critic 的问题列表与建议；针对性补充证据或细化答案
  - 参考：[src/refinement/critic.py:42-58](file://src/refinement/critic.py#L42-L58)
- 幻觉检测触发
  - 检查事实一致性、逻辑连贯性与证据支撑度；增加高质量证据或改进表述
  - 参考：[src/refinement/hallucination.py:54-67](file://src/refinement/hallucination.py#L54-L67)
- 置信度过低
  - 通过 Refiner 基于质量分数微调；必要时补充证据
  - 参考：[src/refinement/refiner.py:52-56](file://src/refinement/refiner.py#L52-L56)
- 记忆修剪未生效
  - 检查阈值设置与记忆项属性；确认 MemoryManager 的删除与更新逻辑
  - 参考：[src/refinement/pruner.py:71-101](file://src/refinement/pruner.py#L71-L101), [src/refinement/pruner.py:120-137](file://src/refinement/pruner.py#L120-L137)
- 异步固化未执行
  - 确认已传入 MemoryManager；检查 run_background_tasks 返回状态
  - 参考：[src/refinement/agent.py:130-150](file://src/refinement/agent.py#L130-L150)

**章节来源**
- [src/refinement/generator.py:45-50](file://src/refinement/generator.py#L45-L50)
- [src/refinement/critic.py:42-58](file://src/refinement/critic.py#L42-L58)
- [src/refinement/hallucination.py:54-67](file://src/refinement/hallucination.py#L54-L67)
- [src/refinement/refiner.py:52-56](file://src/refinement/refiner.py#L52-L56)
- [src/refinement/pruner.py:71-101](file://src/refinement/pruner.py#L71-L101)
- [src/refinement/pruner.py:120-137](file://src/refinement/pruner.py#L120-L137)
- [src/refinement/agent.py:130-150](file://src/refinement/agent.py#L130-L150)

## 结论
精炼代理通过"生成-批判-修正-幻觉检测"的闭环，显著提升了答案的可靠性与可解释性；同时结合知识固化与记忆修剪，保障长期知识质量与系统稳定性。当前模块以最小实现为主，便于快速集成真实LLM与外部存储系统，建议优先完善以下方面：
