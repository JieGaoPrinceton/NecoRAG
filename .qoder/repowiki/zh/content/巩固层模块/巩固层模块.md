...
- 关键点：当前为最小实现，后续需接入查询日志分析、外部知识源与图谱更新逻辑。
...
### 巩固层与知识演化系统的协同机制
- 巩固层在每次处理完成后，会触发记忆管理器的后台任务，执行知识固化与记忆修剪；
- 知识演化系统通过调度器定期执行批量更新，包括向量索引重建、图谱关系维护与大规模知识入库；
- 两者共同确保知识库的持续更新与优化，形成“实时更新 + 定时批量更新”的双重保障。

章节来源
- [src/refinement/agent.py:143-163](file://src/refinement/agent.py#L143-L163)
- [src/knowledge_evolution/README.md:66-81](file://src/knowledge_evolution/README.md#L66-L81)
- [src/knowledge_evolution/config.py:29-40](file://src/knowledge_evolution/config.py#L29-L40)