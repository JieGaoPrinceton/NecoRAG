...
| 模块 | 新增依赖 | 用途 |
|------|---------|------|
| **Intent Analysis** | `jieba>=3.1.0-alpha` | 中文分词与关键词提取 |
| **Domain Weight** | `scipy>=3.1.0-alpha` | 科学计算，相似度计算 |
| **Monitoring** | `prometheus-client>=3.1.0-alpha` | Prometheus 指标收集 |
| **Security** | `PyJWT>=3.1.0-alpha` | JWT 认证 |
| **Security** | `python-jose[cryptography]>=3.1.0-alpha` | OAuth2 支持 |
| **A/B Testing** | `statsmodels>=3.1.0-alpha` | 统计分析模型 |
| **Visualization** | `plotly>=3.1.0-alpha` | 交互式图表 |
| **Visualization** | `matplotlib>=3.1.0-alpha` | 静态图表 |
| **Adaptive Opt.** | `scikit-learn>=3.1.0-alpha` | 机器学习，偏好预测 |
| **Dashboard** | `websockets>=12.0` | WebSocket 实时通信 |
| **Testing** | `pytest-cov>=3.1.0-alpha` | 测试覆盖率统计 |
