...
6. **🆕 Smart Routing Engine** - 智能路由与策略融合（v3.3 新增） ⭐
...
- **智能路由监控**: http://localhost:8000/debug/routing ⭐ (v3.3.0-alpha 新增)
...
results = retriever.retrieve(
    query=query,
    domain_weight_enabled=True  # v3.3.0-alpha 新功能
)
...
## 🔌 RESTful API 使用（v3.3.0-alpha 新增 Interface 模块）
...
decay = MemoryDecay(
    decay_rate=0.1,
    consolidation_interval=300  # v3.3.0-alpha: 支持固化间隔配置
)
...
retriever = AdaptiveRetriever(
    ...
    domain_weight_enabled=True  # v3.3.0-alpha: 启用领域权重计算
)
...
response = interface.respond(
    ...
    include_evidence_sources=True  # v3.3.0-alpha: 包含证据来源追溯
)

# 输出详细的思维链（检索路径 + 证据来源 + 推理过程）
print(response.thinking_chain)
print(response.evidence_sources)  # v3.3.0-alpha 新增
```
...
## 🛠️ 常见问题（v3.3.0-alphaa1.9.0-alpha 更新）
...
### v3.3.0-alphaa1.9.0-alpha 新增模块文档

#### 🆕 v3.3.0-alpha 新增：智能路由与策略融合引擎 ⭐⭐⭐
...
- [项目完成标志](PROJECT_COMPLETE.md) - v3.3.0-alpha/v3.3.0-alpha 完成报告 ⭐
...
**NecoRAG v3.3.0-alpha** | 最后更新：2026-03-19  
