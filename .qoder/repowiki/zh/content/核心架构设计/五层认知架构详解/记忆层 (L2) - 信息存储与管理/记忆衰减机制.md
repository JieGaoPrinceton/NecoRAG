...
#### 权重计算公式
MemoryDecay 类实现了以下数学模型：
- 权重衰减：weight(t) = initial_weight × e^(-λt)
- 访问频率因子：frequency_factor = 1 + log10(access_count + 1)
- 最终权重：final_weight = decayed_weight × frequency_factor
...
1. **时间因素**
   - 指数时间衰减：e^(-λt)，其中 λ 为衰减速率，t 为天数
   - 可配置的时间权重：支持分层、指数、混合三种计算方法

2. **访问频率**
   - 对数频率因子：1 + log10(access_count + 1)
   - 频繁访问的记忆权重会显著提升

3. **内容重要性**
   - 初始权重：由知识库配置决定
   - 领域权重：结合领域权重配置系统
   - 时间权重：根据知识类型（常青/快变）调整

#### 代码示例路径
以下是一些关键实现的代码示例路径：

- 记忆衰减计算：[src/memory/decay.py:39-70](file://src/memory/decay.py#L39-L70)
- 批量衰减应用：[src/memory/decay.py:72-94](file://src/memory/decay.py#L72-L94)
- 归档阈值判断：[src/memory/decay.py:96-118](file://src/memory/decay.py#L96-L118)
- 记忆强化机制：[src/memory/decay.py:120-142](file://src/memory/decay.py#L120-L142)
- 主动遗忘实现：[src/memory/manager.py:184-202](file://src/memory/manager.py#L184-L202)

#### 最佳实践建议

1. **记忆稳定性平衡**
   - 对于重要知识：提高 archive_threshold，减少误删风险
   - 对于临时知识：降低 archive_threshold，及时释放存储空间
   - 定期审查：通过健康指标监控知识库状态
...
3. **性能优化**
   - 批量处理：利用 apply_decay 进行批量衰减
   - 缓存策略：合理设置缓存时间，避免重复计算
   - 监控告警：建立健康指标监控，及时发现问题

章节来源
- [src/memory/decay.py:11-155](file://src/memory/decay.py#L11-L155)
- [src/memory/manager.py:149-202](file://src/memory/manager.py#L149-L202)
- [src/domain/temporal_weight.py:160-270](file://src/domain/temporal_weight.py#L160-L270)
- [src/knowledge_evolution/metrics.py:508-572](file://src/knowledge_evolution/metrics.py#L508-L572)