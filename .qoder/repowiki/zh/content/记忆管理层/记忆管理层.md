...
- [wiki/wiki/记忆管理层/L2语义记忆（向量数据库）.md](file://wiki/wiki/记忆管理层/L2语义记忆（向量数据库）.md)
- [wiki/wiki/记忆管理层/L3情景图谱（图数据库）.md](file://wiki/wiki/记忆管理层/L3情景图谱（图数据库）.md)
</cite>
...
## 简介
本文件系统阐述 NecoRAG 记忆管理层的三层记忆架构：L1 工作记忆（Redis）、L2 语义记忆（向量数据库）、L3 情景图谱（图数据库），并深入解析记忆衰减与巩固机制、数据模型、访问模式与缓存策略。文档旨在帮助开发者与产品人员快速理解并扩展该记忆系统。
