# 生产环境部署

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [QUICKSTART.md](file://QUICKSTART.md)
- [pyproject.toml](file://pyproject.toml)
- [requirements.txt](file://requirements.txt)
- [src/core/config.py](file://src/core/config.py)
- [src/dashboard/server.py](file://src/dashboard/server.py)
- [src/dashboard/config_manager.py](file://src/dashboard/config_manager.py)
- [src/dashboard/models.py](file://src/dashboard/models.py)
- [tools/start_dashboard.py](file://tools/start_dashboard.py)
- [src/memory/manager.py](file://src/memory/manager.py)
- [src/retrieval/retriever.py](file://src/retrieval/retriever.py)
- [src/refinement/agent.py](file://src/refinement/agent.py)
- [src/response/interface.py](file://src/response/interface.py)
- [opdev/docker-compose.yml](file://opdev/docker-compose.yml)
- [opdev/docker-compose.dev.yml](file://opdev/docker-compose.dev.yml)
- [opdev/docker-compose.minimal.yml](file://opdev/docker-compose.minimal.yml)
- [opdev/Dockerfile](file://opdev/Dockerfile)
- [opdev/scripts/start.sh](file://opdev/scripts/start.sh)
- [opdev/scripts/stop.sh](file://opdev/scripts/stop.sh)
- [opdev/scripts/status.sh](file://opdev/scripts/status.sh)
</cite>

## 更新摘要
**所做更改**
- 新增Docker Compose多环境部署配置章节，详细介绍四种部署模式
- 更新Docker容器化部署方案，包含开发模式、最小模式、完整模式和LLM模式
- 新增Docker Compose配置文件分析和环境变量管理说明
- 补充Docker环境管理脚本的使用指南
- 更新部署检查清单，包含新的部署模式验证步骤

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [Docker Compose多环境部署](#docker-compose多环境部署)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件面向运维团队，提供NecoRAG框架在生产环境中的完整部署指南。内容涵盖硬件要求、软件依赖、网络配置、安全设置、容器化与Kubernetes部署、负载均衡与高可用、容灾备份、环境变量与配置管理、启动脚本使用以及性能调优与资源限制的最佳实践，并配套部署检查清单与验证步骤。

**更新** 新增Docker Compose多环境部署配置，提供开发模式(dev)、最小模式(minimal)、完整模式(full)和LLM模式(--with-llm)四种灵活的部署选项。

## 项目结构
NecoRAG采用模块化分层架构，核心由感知层、记忆层、检索层、巩固层与交互层组成。Dashboard提供Web配置管理与监控能力，支持Profile配置的创建、切换、导入导出与实时参数调整。

```mermaid
graph TB
subgraph "应用层"
UI["Dashboard Web UI"]
API["RESTful API"]
end
subgraph "业务层"
Perception["感知引擎<br/>文档解析与编码"]
Memory["层级记忆<br/>L1/L2/L3"]
Retrieval["自适应检索<br/>HyDE/重排序/早停"]
Refinement["精炼代理<br/>幻觉检测/知识固化"]
Response["响应接口<br/>情境自适应输出"]
end
subgraph "数据层"
Redis["Redis L1 工作记忆"]
Qdrant["Qdrant/Milvus L2 语义记忆"]
Neo4j["Neo4j/NebulaGraph L3 情景图谱"]
end
UI --> API
API --> Perception
API --> Memory
API --> Retrieval
API --> Refinement
API --> Response
Memory --> Redis
Memory --> Qdrant
Memory --> Neo4j
```

**图表来源**
- [README.md:35-85](file://README.md#L35-L85)
- [src/memory/manager.py:16-47](file://src/memory/manager.py#L16-L47)

**章节来源**
- [README.md:23-85](file://README.md#L23-L85)
- [src/memory/manager.py:16-47](file://src/memory/manager.py#L16-L47)

## 核心组件
- 配置管理：统一的全局配置类与模块配置基类，支持从文件与环境变量加载，提供开发/生产/最小化预设配置。
- Dashboard：基于FastAPI的Web管理界面，提供Profile管理、模块参数配置、统计监控与API接口。
- 记忆层：三层记忆系统（L1工作记忆、L2语义记忆、L3情景图谱），支持衰减与归档。
- 检索层：混合检索与重排序，集成HyDE增强、Novelty重排与早停机制。
- 巩固层：Generator-Critic-Refiner闭环，支持幻觉检测与知识固化。
- 响应层：情境自适应生成与思维链可视化。

**章节来源**
- [src/core/config.py:232-370](file://src/core/config.py#L232-L370)
- [src/dashboard/server.py:43-93](file://src/dashboard/server.py#L43-L93)
- [src/memory/manager.py:16-47](file://src/memory/manager.py#L16-L47)
- [src/retrieval/retriever.py:122-164](file://src/retrieval/retriever.py#L122-L164)
- [src/refinement/agent.py:16-60](file://src/refinement/agent.py#L16-L60)
- [src/response/interface.py:16-54](file://src/response/interface.py#L16-L54)

## 架构概览
生产部署建议采用微服务架构，将Dashboard作为独立服务，业务组件通过API交互；数据层使用外部数据库（Redis/Qdrant/Neo4j）以支持横向扩展与高可用。

```mermaid
graph TB
LB["负载均衡器<br/>Nginx/HAProxy"]
subgraph "应用节点"
D1["Dashboard 1"]
D2["Dashboard 2"]
S1["业务服务 1"]
S2["业务服务 2"]
end
subgraph "数据层"
R["Redis 集群"]
Q["Qdrant 集群"]
G["Neo4j 集群"]
end
LB --> D1
LB --> D2
LB --> S1
LB --> S2
S1 --> R
S1 --> Q
S1 --> G
S2 --> R
S2 --> Q
S2 --> G
```

**图表来源**
- [README.md:69-75](file://README.md#L69-L75)
- [src/memory/manager.py:23-43](file://src/memory/manager.py#L23-L43)

## 详细组件分析

### 配置管理与环境变量
- 支持从配置文件与环境变量加载，环境变量优先级高于配置文件。
- 关键环境变量前缀为NECORAG，可覆盖LLM、向量数据库、图数据库等配置。
- 提供开发/生产/最小化预设配置，便于快速切换。

```mermaid
flowchart TD
Start(["启动"]) --> LoadFile["加载配置文件"]
LoadFile --> EnvOverride["环境变量覆盖"]
EnvOverride --> Apply["应用配置"]
Apply --> End(["完成"])
```

**图表来源**
- [src/core/config.py:288-327](file://src/core/config.py#L288-L327)

**章节来源**
- [src/core/config.py:288-327](file://src/core/config.py#L288-L327)
- [src/core/config.py:340-370](file://src/core/config.py#L340-L370)

### Dashboard 部署与API
- 提供RESTful API与Web UI，支持Profile管理、模块参数配置与统计监控。
- 支持CORS配置，便于前端直连。
- 启动脚本支持指定host/port/config-dir参数。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Server as "DashboardServer"
participant CM as "ConfigManager"
participant FS as "文件系统"
Client->>Server : GET /api/profiles
Server->>CM : 获取所有Profile
CM->>FS : 读取配置文件
FS-->>CM : 返回Profile数据
CM-->>Server : 返回Profile列表
Server-->>Client : 200 OK + JSON
Client->>Server : POST /api/profiles
Server->>CM : 创建Profile
CM->>FS : 写入配置文件
FS-->>CM : 确认写入
CM-->>Server : 返回新Profile
Server-->>Client : 200 OK + JSON
```

**图表来源**
- [src/dashboard/server.py:94-180](file://src/dashboard/server.py#L94-L180)
- [src/dashboard/config_manager.py:42-74](file://src/dashboard/config_manager.py#L42-L74)

**章节来源**
- [src/dashboard/server.py:43-93](file://src/dashboard/server.py#L43-L93)
- [src/dashboard/server.py:94-180](file://src/dashboard/server.py#L94-L180)
- [src/dashboard/config_manager.py:14-41](file://src/dashboard/config_manager.py#L14-L41)
- [tools/start_dashboard.py:16-51](file://tools/start_dashboard.py#L16-L51)

### 记忆层与数据存储
- L1工作记忆：Redis，支持TTL与容量限制。
- L2语义记忆：Qdrant/Milvus，支持向量检索与索引。
- L3情景图谱：Neo4j/NebulaGraph，支持实体关系网络与多跳推理。
- 记忆衰减与归档：根据权重阈值进行主动遗忘与持久化。

```mermaid
classDiagram
class MemoryManager {
+store(chunk) str
+retrieve(query, query_vector, layers, top_k) List
+consolidate() void
+forget(threshold) int
}
class WorkingMemory {
+存储与检索
}
class SemanticMemory {
+向量存储与搜索
}
class EpisodicGraph {
+实体与关系管理
}
class MemoryDecay {
+衰减计算
+归档判定
}
MemoryManager --> WorkingMemory : "使用"
MemoryManager --> SemanticMemory : "使用"
MemoryManager --> EpisodicGraph : "使用"
MemoryManager --> MemoryDecay : "使用"
```

**图表来源**
- [src/memory/manager.py:16-47](file://src/memory/manager.py#L16-L47)

**章节来源**
- [src/memory/manager.py:16-47](file://src/memory/manager.py#L16-L47)

### 检索层与早停机制
- 混合检索：向量检索 + 图谱检索 + 结果融合。
- HyDE增强：生成假设文档提升检索质量。
- 重排序：BGE-Reranker-v2进行精排。
- 早停机制：基于置信度与边际收益快速终止冗余检索。

```mermaid
flowchart TD
Q["输入查询"] --> E["查询分析"]
E --> V["向量检索"]
E --> G["图谱检索"]
V --> F["结果融合"]
G --> F
F --> R["重排序"]
R --> W["应用领域权重"]
W --> Filter["过滤低分结果"]
Filter --> C["评估置信度"]
C --> Terminate{"是否满足早停条件？"}
Terminate --> |是| Return["返回结果"]
Terminate --> |否| Continue["继续检索"]
Continue --> V
```

**图表来源**
- [src/retrieval/retriever.py:177-254](file://src/retrieval/retriever.py#L177-L254)

**章节来源**
- [src/retrieval/retriever.py:122-164](file://src/retrieval/retriever.py#L122-L164)
- [src/retrieval/retriever.py:177-254](file://src/retrieval/retriever.py#L177-L254)

### 巩固层与幻觉检测
- Generator-Critic-Refiner闭环：生成-批判-修正。
- 幻觉检测：基于事实一致性、证据支撑度与逻辑连贯性。
- 知识固化与记忆修剪：异步执行，定期清理低价值记忆。

```mermaid
sequenceDiagram
participant QA as "查询"
participant Gen as "Generator"
participant Cri as "Critic"
participant Ref as "Refiner"
participant HD as "幻觉检测"
participant KC as "知识固化"
participant MP as "记忆修剪"
QA->>Gen : 生成初始答案
Gen-->>Cri : 提交答案与证据
Cri-->>Gen : 批判反馈
Gen->>HD : 幻觉检测
HD-->>Gen : 检测结果
alt 通过验证
Gen-->>QA : 返回最终答案
else 未通过
Gen->>Ref : 修正答案
Ref-->>Gen : 修正结果
Gen->>KC : 触发固化
KC-->>MP : 触发修剪
end
```

**图表来源**
- [src/refinement/agent.py:61-129](file://src/refinement/agent.py#L61-L129)

**章节来源**
- [src/refinement/agent.py:16-60](file://src/refinement/agent.py#L16-L60)
- [src/refinement/agent.py:61-129](file://src/refinement/agent.py#L61-L129)

### 响应层与思维链可视化
- 用户画像适配：根据专业水平与交互风格调整语气与详细程度。
- 思维链可视化：展示检索路径、证据来源与推理过程。
- 多模态输出：支持文本/Markdown/HTML格式。

```mermaid
classDiagram
class ResponseInterface {
+respond(query, refinement_result, session_id, tone, detail_level) Response
-_determine_detail_level(query, profile, result) int
-_generate_thinking_chain(query, result) str
}
class UserProfileManager {
+get_profile(user_id) UserProfile
+update_profile(user_id, interaction) void
+analyze_preference(user_id) dict
}
class ToneAdapter {
+adapt(content, tone) str
+inject_personality(content, tone) str
}
class DetailLevelAdapter {
+adapt(content, level) str
}
class ThinkingChainVisualizer {
+visualize(trace, evidence, reasoning) str
}
ResponseInterface --> UserProfileManager : "使用"
ResponseInterface --> ToneAdapter : "使用"
ResponseInterface --> DetailLevelAdapter : "使用"
ResponseInterface --> ThinkingChainVisualizer : "使用"
```

**图表来源**
- [src/response/interface.py:16-54](file://src/response/interface.py#L16-L54)

**章节来源**
- [src/response/interface.py:16-54](file://src/response/interface.py#L16-L54)
- [src/response/interface.py:167-211](file://src/response/interface.py#L167-L211)

## Docker Compose多环境部署

### 部署模式概述
NecoRAG提供四种Docker Compose部署模式，满足不同场景的部署需求：

- **开发模式(dev)**：仅启动后台服务，应用容器默认不启动，适合开发者本地运行
- **最小模式(minimal)**：仅启动核心存储服务(Redis + Qdrant)
- **完整模式(full)**：启动所有服务，包括Dashboard、数据存储和监控
- **LLM模式(--with-llm)**：完整模式 + LLM推理引擎(Ollama)

### Docker Compose配置分析

#### 核心服务配置
Docker Compose文件定义了完整的NecoRAG服务架构：

```mermaid
graph TB
subgraph "数据存储层"
Redis["Redis L1<br/>6379/TCP"]
Qdrant["Qdrant L2<br/>6333/6334/TCP"]
Neo4j["Neo4j L3<br/>7474/7687/TCP"]
end
subgraph "推理服务层"
Ollama["Ollama LLM<br/>11434/TCP"]
end
subgraph "监控层"
Grafana["Grafana<br/>3000/TCP"]
end
subgraph "应用层"
Dashboard["NecoRAG Dashboard<br/>8000/TCP"]
end
Redis --> Dashboard
Qdrant --> Dashboard
Neo4j --> Dashboard
Ollama --> Dashboard
Dashboard --> Grafana
```

**图表来源**
- [opdev/docker-compose.yml:4-164](file://opdev/docker-compose.yml#L4-L164)

#### 环境变量配置
Docker Compose通过环境变量实现灵活的配置管理：

- **数据存储配置**：REDIS_URL、QDRANT_URL、NEO4J_URL
- **LLM配置**：NECORAG_LLM_PROVIDER、NECORAG_LLM_API_BASE
- **调试配置**：NECORAG_DEBUG
- **端口映射**：通过环境变量控制容器端口映射

**章节来源**
- [opdev/docker-compose.yml:130-139](file://opdev/docker-compose.yml#L130-L139)

### 部署模式详解

#### 开发模式(dev)
开发模式通过profiles实现按需启动，适合开发环境：

- **应用容器**：默认不启动，开发者可直接使用本地环境
- **LLM服务**：可选启动，通过`--profile llm`启用
- **监控服务**：可选启动，通过`--profile monitoring`启用

**章节来源**
- [opdev/docker-compose.dev.yml:4-16](file://opdev/docker-compose.dev.yml#L4-L16)

#### 最小模式(minimal)
最小模式专注于核心存储服务，适合轻量部署：

- **Redis**：基础工作记忆存储
- **Qdrant**：语义向量检索
- **简化配置**：移除了Neo4j、Ollama、Grafana等服务

**章节来源**
- [opdev/docker-compose.minimal.yml:1-33](file://opdev/docker-compose.minimal.yml#L1-L33)

#### 完整模式(full)
完整模式提供生产环境标准配置：

- **所有核心服务**：Redis、Qdrant、Neo4j
- **Dashboard**：Web管理界面
- **监控系统**：Grafana可视化
- **健康检查**：每个服务都配置了健康检查

#### LLM模式(--with-llm)
LLM模式在完整模式基础上添加推理引擎：

- **Ollama服务**：本地大语言模型推理
- **GPU支持**：可选的NVIDIA GPU加速配置
- **模型管理**：支持多种预训练模型

**章节来源**
- [opdev/docker-compose.yml:73-97](file://opdev/docker-compose.yml#L73-L97)

### Docker环境管理脚本

#### 启动脚本(start.sh)
提供便捷的部署模式选择：

```bash
# 基本用法
./scripts/start.sh              # 启动完整模式
./scripts/start.sh dev          # 启动开发模式
./scripts/start.sh minimal      # 启动最小模式
./scripts/start.sh --with-llm   # 启动完整模式 + LLM
```

**章节来源**
- [opdev/scripts/start.sh:1-101](file://opdev/scripts/start.sh#L1-L101)

#### 停止脚本(stop.sh)
支持清理模式和普通停止：

```bash
./scripts/stop.sh              # 停止所有服务
./scripts/stop.sh --clean      # 停止并清理数据卷
```

**章节来源**
- [opdev/scripts/stop.sh:1-36](file://opdev/scripts/stop.sh#L1-L36)

#### 状态检查脚本(status.sh)
提供全面的服务状态检查：

- **容器状态**：Docker容器运行状态
- **服务连通性**：各服务端口可达性检查
- **数据卷状态**：存储卷使用情况

**章节来源**
- [opdev/scripts/status.sh:1-48](file://opdev/scripts/status.sh#L1-L48)

### Docker容器化部署方案

#### 基础镜像与依赖
Dockerfile基于Python 3.11 Slim镜像构建：

- **系统依赖**：build-essential、curl等编译工具
- **Python依赖**：通过requirements.txt安装
- **源码复制**：src/和tools/目录的完整复制

**章节来源**
- [opdev/Dockerfile:1-39](file://opdev/Dockerfile#L1-L39)

#### 端口暴露与健康检查
- **端口暴露**：8000/TCP用于Dashboard服务
- **健康检查**：通过curl检查/api/stats端点
- **数据卷**：/app/data、/app/configs、/app/logs目录

#### 环境变量管理
Docker Compose通过环境变量实现配置管理：

- **数据存储连接**：REDIS_URL、QDRANT_URL、NEO4J_URL
- **LLM配置**：NECORAG_LLM_PROVIDER、NECORAG_LLM_API_BASE
- **调试模式**：NECORAG_DEBUG控制调试输出

**章节来源**
- [opdev/docker-compose.yml:130-139](file://opdev/docker-compose.yml#L130-L139)

## 依赖分析
- Python版本：3.9+
- 核心依赖：numpy、python-dateutil
- Dashboard依赖：fastapi、uvicorn、pydantic
- 可选依赖：文档解析、向量数据库、图数据库、缓存、嵌入模型、LLM集成、NLP工具、aiohttp、python-dotenv、requests

```mermaid
graph LR
A["NecoRAG 核心"] --> B["numpy"]
A --> C["python-dateutil"]
A --> D["fastapi"]
A --> E["uvicorn"]
A --> F["pydantic"]
A -.可选.-> G["ragflow"]
A -.可选.-> H["qdrant-client/milvus"]
A -.可选.-> I["neo4j/nebula3"]
A -.可选.-> J["redis"]
A -.可选.-> K["langchain/langgraph/openai"]
A -.可选.-> L["spacy/transformers"]
```

**图表来源**
- [requirements.txt:3-47](file://requirements.txt#L3-L47)

**章节来源**
- [requirements.txt:3-47](file://requirements.txt#L3-L47)
- [pyproject.toml:27-30](file://pyproject.toml#L27-L30)

## 性能考虑
- 检索性能：合理设置top_k、confidence_threshold与rerank_top_k，启用早停机制减少无效计算。
- 记忆层优化：为Redis设置合适的TTL与最大项数，为Qdrant配置合适索引类型与HNSW参数。
- LLM调优：根据延迟与成本平衡temperature与max_tokens，选择合适的嵌入模型维度。
- 并发与资源：为Dashboard与业务服务设置合理的并发数与内存上限，避免资源争用。
- 监控与日志：开启详细日志与性能指标，定期分析慢查询与异常。

## 故障排除指南
- Dashboard启动失败：检查端口占用，确认host/port参数正确；查看日志定位具体错误。
- 配置加载异常：确认环境变量命名与前缀一致，检查配置文件JSON格式；验证枚举值有效性。
- 记忆层连接失败：核对Redis/Qdrant/Neo4j连接URL与认证信息；确保网络连通性与防火墙放行。
- 检索结果不理想：调整confidence_threshold、rerank_top_k与领域权重因子；启用HyDE与重排序。
- 幻觉检测触发频繁：提高hallucination_threshold或增加证据来源；优化生成提示词。
- Docker部署问题：检查Docker服务状态，确认compose文件语法正确，验证端口映射配置。

**章节来源**
- [QUICKSTART.md:245-259](file://QUICKSTART.md#L245-L259)
- [src/core/config.py:288-327](file://src/core/config.py#L288-L327)
- [src/memory/manager.py:23-43](file://src/memory/manager.py#L23-L43)
- [src/retrieval/retriever.py:150-151](file://src/retrieval/retriever.py#L150-L151)

## 结论
通过模块化的配置管理、清晰的组件边界与可扩展的数据层，NecoRAG可在生产环境中实现稳定、高性能与易维护的部署。结合负载均衡、高可用与容灾备份策略，可进一步提升系统的可靠性与可伸缩性。新增的Docker Compose多环境部署配置提供了更加灵活的部署选项，满足从开发到生产的各种场景需求。

## 附录

### 硬件要求建议
- CPU：至少8核，推荐16核以上
- 内存：至少32GB，推荐64GB以上
- 存储：SSD至少500GB，建议为数据层单独挂载高IO磁盘
- 网络：千兆以太网，内网低延迟

### 软件依赖安装
- Python 3.9+
- Docker与Docker Compose
- 依赖安装：pip install -r requirements.txt
- Dashboard运行：python tools/start_dashboard.py

**章节来源**
- [requirements.txt:1-13](file://requirements.txt#L1-L13)
- [QUICKSTART.md:5-13](file://QUICKSTART.md#L5-L13)
- [tools/start_dashboard.py:16-51](file://tools/start_dashboard.py#L16-L51)

### 网络配置与安全
- Dashboard监听：默认0.0.0.0:8000，生产环境建议绑定内网IP并启用反向代理
- CORS：允许跨域访问，生产环境建议限制来源域名
- TLS：为Dashboard与业务服务配置HTTPS证书
- 防火墙：仅开放Dashboard与业务服务端口，限制Redis/Qdrant/Neo4j访问范围

**章节来源**
- [src/dashboard/server.py:54-87](file://src/dashboard/server.py#L54-L87)
- [src/dashboard/server.py:239-253](file://src/dashboard/server.py#L239-L253)

### 环境变量配置清单
- NECORAG_DEBUG：布尔值，启用调试模式
- NECORAG_LLM_PROVIDER：LLM提供商（mock/openai/ollama/vllm/azure/anthropic）
- NECORAG_LLM_MODEL：模型名称
- NECORAG_LLM_API_KEY：API密钥（按提供商命名）
- NECORAG_VECTOR_DB：向量数据库提供商（memory/qdrant/milvus/chroma）
- NECORAG_VECTOR_DB_URL：向量数据库连接URL
- NECORAG_GRAPH_DB：图数据库提供商（memory/neo4j/nebula）
- NECORAG_GRAPH_DB_URL：图数据库连接URL

**章节来源**
- [src/core/config.py:288-327](file://src/core/config.py#L288-L327)

### 配置文件管理
- Profile创建与激活：通过Dashboard或API创建并激活配置
- 参数更新：支持逐模块参数更新与批量导入导出
- 统计监控：实时显示文档总数、块总数、查询总数与活动会话

**章节来源**
- [src/dashboard/config_manager.py:42-74](file://src/dashboard/config_manager.py#L42-L74)
- [src/dashboard/server.py:181-236](file://src/dashboard/server.py#L181-L236)

### 启动脚本使用
- start_dashboard.py：支持host/port/config-dir参数
- 支持Windows批处理与Linux/Mac Shell脚本启动方式
- Docker部署：使用opdev/scripts/start.sh管理不同部署模式

**章节来源**
- [tools/start_dashboard.py:16-51](file://tools/start_dashboard.py#L16-L51)
- [README.md:138-152](file://README.md#L138-L152)
- [opdev/scripts/start.sh:1-101](file://opdev/scripts/start.sh#L1-L101)

### Docker容器化部署方案
- 基础镜像：python:3.9-slim
- 依赖安装：COPY requirements.txt && RUN pip install -r requirements.txt
- 应用启动：CMD ["python", "tools/start_dashboard.py"]
- 端口暴露：EXPOSE 8000
- 环境变量：通过docker run -e传入NECORAG_*变量
- 数据卷：挂载配置目录与日志目录

### Kubernetes集群部署指南
- Deployment：Dashboard与业务服务分别部署
- Service：ClusterIP或LoadBalancer暴露服务
- ConfigMap：存放配置文件与环境变量
- Secret：存放API密钥与数据库密码
- PersistentVolume：为配置与日志提供持久化存储
- HPA：根据CPU/内存或自定义指标自动扩缩容

### 负载均衡、高可用与容灾
- 负载均衡：Nginx/HAProxy轮询或健康检查
- 高可用：Dashboard与业务服务多副本部署，共享配置中心
- 容灾：数据层使用集群模式，定期备份与异地容灾演练

### 性能调优与资源限制最佳实践
- Dashboard：设置并发worker数与连接池大小，限制内存使用
- 业务服务：根据查询复杂度调整top_k与重排序参数，启用早停
- 数据层：为Redis设置合理的TTL与淘汰策略，为Qdrant配置索引参数
- LLM：根据SLA设定超时与重试策略，使用流式响应降低首字延迟

### 部署检查清单

#### 基础部署检查
- 系统资源：CPU/内存/存储满足最低要求
- 依赖安装：requirements.txt中依赖全部安装成功
- Docker环境：Docker服务正常运行，Compose版本兼容
- 环境变量：NECORAG_*变量配置正确且生效

#### 不同部署模式验证
- **开发模式(dev)**：后台服务正常运行，应用容器可手动启动
- **最小模式(minimal)**：Redis和Qdrant服务正常，Dashboard可访问
- **完整模式(full)**：所有核心服务正常，Grafana监控可用
- **LLM模式(--with-llm)**：Ollama服务正常，可拉取和使用模型

#### 网络与安全检查
- 网络连通：Dashboard与数据层服务间网络连通
- 端口映射：各服务端口正确映射到主机
- 安全配置：TLS、CORS、防火墙规则已配置
- 高可用：多副本部署与健康检查已启用

#### 监控与备份检查
- 监控：日志与性能指标采集已部署
- 备份：数据层与配置文件定期备份策略
- 健康检查：所有服务配置了健康检查

### 验证步骤
- Dashboard可用性：访问http://host:8000与/docs，确认UI与API正常
- Profile管理：创建/激活/更新Profile，验证参数生效
- 检索验证：构造查询，确认检索结果与早停机制正常
- 记忆层验证：确认L1/L2/L3存储与查询正常
- 幻觉检测：构造容易产生幻觉的问题，确认检测与修正流程
- 性能验证：压测Dashboard与业务服务，观察延迟与吞吐
- Docker部署验证：使用opdev/scripts/status.sh检查服务状态

**章节来源**
- [README.md:138-157](file://README.md#L138-L157)
- [src/dashboard/server.py:379-392](file://src/dashboard/server.py#L379-L392)
- [opdev/scripts/status.sh:1-48](file://opdev/scripts/status.sh#L1-L48)