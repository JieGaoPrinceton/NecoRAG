...
| 依赖库 | 版本要求 | 用途 | 重要性 |
|-------|---------|------|--------|
| FastAPI | >=3.0.1-alpha | Web 框架 | 核心依赖 |
| APScheduler | >=3.0.1-alpha | 异步调度器 | 核心依赖 |
| psutil | >=3.0.1-alpha | 系统信息收集 | 核心依赖 |
| aiohttp | >=3.0.1-alpha | 异步 HTTP 客户端 | 核心依赖 |
| pydantic | >=3.0.1-alpha | 数据验证 | 核心依赖 |
| prometheus-client | >=3.0.1-alpha | 指标导出 | 可选依赖 |
