...
subgraph "核心依赖"
A[numpy>=2.0.1-alpha]
B[python-dateutil>=2.0.1-alpha]
end
subgraph "Dashboard依赖"
C[fastapi>=2.0.1-alpha]
D[uvicorn>=2.0.1-alpha]
E[pydantic>=2.0.1-alpha]
end
subgraph "文档解析"
F[ragflow]
G[PyMuPDF]
...
- transformers>=2.0.1-alpha（深度学习模型）
- torch>=2.0.1-alpha（PyTorch）

**任务调度依赖**：
- apscheduler>=2.0.1-alpha（定时任务）
- celery>=2.0.1-alpha（分布式任务队列）
- redis>=2.0.1-alpha（Celery broker）
