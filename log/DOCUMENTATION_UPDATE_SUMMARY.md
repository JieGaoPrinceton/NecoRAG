...
全面反映 NecoRAG v3.2.0-alpha 的最新状态，包括：
...
3. 思维链可视化
4. 幻觉自检闭环
5. **领域权重计算**（v3.2.0-alpha 新增）⭐
6. **意图分析系统**（v3.2.0-alpha 新增）⭐
...
**分为两部分**：
- 核心依赖（7 个项目）
- v3.2.0-alpha 新增感谢（5 个项目：Prometheus, Grafana, PyJWT, SciPy, Plotly）
...
**新增内容**：
- v3.2.0-alpha 新增模块的配置说明（8 个新模块 Tab）
...
**更新的问题**：
- Q1: 依赖安装（区分核心依赖和 v3.2.0-alpha 新功能依赖）
...
- 新增 8 个 v3.2.0-alpha 模块文档链接
- 新增调试面板文档链接

### 10. 下一步规划更新

**分为三类**：
- ✅ 已完成（v3.2.0-alpha）- 列出所有已完成的功能
...
- `jieba>=3.2.0-alpha` - 中文分词
- `scipy>=3.2.0-alpha` - 科学计算
- `prometheus-client>=3.2.0-alpha` - Prometheus 指标收集
- `PyJWT>=3.2.0-alpha` - JWT 认证
- `python-jose[cryptography]>=3.2.0-alpha` - OAuth2 支持
- `statsmodels>=3.2.0-alpha` - 统计分析模型
- `plotly>=3.2.0-alpha` - 交互式图表
- `matplotlib>=3.2.0-alpha` - 静态图表
- `scikit-learn>=3.2.0-alpha` - 机器学习
- `pytest-cov>=3.2.0-alpha` - 测试覆盖率统计
- `passlib[bcrypt]>=3.2.0-alpha` - 密码加密
...
### 1. 信息完整性
- ✅ 覆盖了所有 v3.2.0-alpha 新增的 8 个核心模块
- ✅ 详细说明了可视化调试面板的 7 大功能
- ✅ 提供了完整的依赖清单和安装指南
...
[![Version](https://img.shields.io/badge/version-3.2.0-alpha--alpha-blue.svg)](CHANGELOG.md)
