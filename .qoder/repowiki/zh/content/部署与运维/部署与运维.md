...
MS["src/monitoring/service.py"]
MM["src/monitoring/metrics.py"]
SC["src/security/config.py"]
SA["src/security/auth.py"]
end
DF --> DC
DC --> MC
DC --> SC
...
MonitoringService --> MonitoringConfig : "读取配置"
...
## 结论
通过标准化的容器镜像与 Compose 编排，NecoRAG 可在开发与生产环境中快速部署并保持一致的行为。配合完善的监控与安全配置，能够有效保障系统的可观测性与安全性。运维脚本简化了日常操作，结合合理的性能调优与故障恢复策略，可构建稳定可靠的生产环境。
