...
[![Version](https://img.shields.io/badge/version-3.2.0-alpha-blue.svg)](CHANGELOG.md)
...
**当前版本**: v3.2.0-alpha | **最后更新**: 2026-03-19  
...
- 🔍 **可视化调试面板**：思维路径可视化、实时监控、A/B 测试（v3.2.0-alpha 新增）⭐
...
### 🆕 新增核心模块（v3.2.0-alphaa1.9.0-alpha）

```
┌─────────────────────────────────────────────────────────┐
│         NecoRAG v3.2.0-alpha 新增：智能路由与策略融合引擎       │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  🧠 智能路由与策略融合引擎 (smart_routing/)             │
│     • 意图路由器（7 大类语义意图）                       │
│     • 用户画像适配器（专业度适配）                      │
│     • CoT 控制器（深度动态调节）                         │
│     • 策略融合引擎（多策略并行）                        │
...
- **Python**: >= 3.9 (推荐 3.2.0-alpha)
- **操作系统**: Linux / macOS / Windows
- **内存**: 最低 4GB，推荐 8GB+
- **存储**: 至少 2GB 可用空间

**详细的环境配置指南请参考**: [DEVELOPMENT_ENVIRONMENT.md](DEVELOPMENT_ENVIRONMENT.md) ⭐
...
详细文档：[devops/README.md](devops/README.md)
...
# v3.2: 初始化智能路由引擎
strategy_engine = StrategyFusionEngine(
...
- **调试面板**: http://localhost:8000/debug (v3.2.0-alpha 新增) ⭐

### 使用 Interface 模块（v3.2.0-alpha 新增）
...
| domain_weight_enabled | bool | True | 启用领域权重（v3.2.0-alpha） |
...
| evolution_enabled | bool | True | 启用知识演化（v3.2.0-alpha） |
...
# WebSocket 实时推送 (v3.2.0-alpha 新增)
WS /ws/app
WS /ws/thinking-path
```
...
| **调试面板覆盖率** | 100% | ✅ 完成 | v3.2.0-alpha 新增 |
| **智能路由引擎** | 完整 | ✅ 完成 | v3.2.0-alpha 新增 ⭐ |
| **代码质量** | 良好 | ✅ 80.2% | 代码密度 |
| **文档完整度** | 优秀 | ✅ 16.6 万 + 行 | Markdown 文档 |
...
- 🔄 动态重排序与 Novelty 检测
- 🔄 意图分析系统（v3.2.0-alpha 已完成）⭐
- 🔄 领域权重系统（v3.2.0-alpha 已完成）⭐
- 🔄 知识演化系统（v3.2.0-alpha 已完成）⭐
- 🔄 监控告警系统（v3.2.0-alpha 已完成）⭐
- 🔄 安全模块（v3.2.0-alpha 已完成）⭐
- 🔄 自适应优化（v3.2.0-alpha 已完成）⭐
- 🔄 插件扩展系统（v3.2.0-alpha 已完成）⭐
- 🔄 Interface 模块（v3.2.0-alpha 已完成）⭐
- ✅ **智能路由与策略融合引擎**（v3.2.0-alpha 已完成）⭐⭐⭐
...
- 🚀 异步知识固化与自动遗忘
- 📊 **可视化调试面板** (NecoRAG Dashboard) - ✅ v3.2.0-alpha 已完成 ⭐
- 🔌 插件市场和扩展系统 - ✅ 框架已完成 ⭐
- 🌍 社区运营和开源生态建设
- 🏢 企业级功能增强（多租户、审计、报表）
- 📱 移动端应用开发
...
### 🆕 v3.2.0-alpha 新增依赖（智能路由引擎）

```bash
# 智能路由与策略融合引擎已包含在核心库中，无需额外依赖
# 以下依赖已在 requirements.txt 中自动包含：
# - 意图分类与语义分析（使用现有 LLM 集成）
# - 用户画像建模（使用现有数据存储）
# - CoT思维链控制（内置算法）
```
...
### 🆕 新增模块文档（v3.2.0-alphaa1.9.0-alpha）

#### 智能路由与策略融合（v3.2 新增）
- [Smart Routing Engine](src/retrieval/smart_routing/README.md) - 智能路由引擎 ⭐⭐⭐
- [Implementation Summary](src/retrieval/smart_routing/IMPLEMENTATION_SUMMARY.md) - 实现总结 ⭐

#### 其他新增模块文档（v3.2.0-alpha）
