...
## 简介
本文件面向知识演化调度管理器，系统性阐述定时任务的调度策略与执行机制，涵盖批处理任务的创建、状态管理与执行监控；深入解析调度器配置项（执行频率、优先级、并发控制等）；展示任务队列管理（排队、优先级排序、资源分配策略）；阐明调度器与知识更新流程的集成方式（自动触发与手动干预）；并提供任务生命周期管理与故障恢复策略。调度管理器在 v3.3.0-alpha 版本中保持了稳定的内核实现，同时为后续引入专业调度框架（如 APScheduler/Celery）预留了扩展点。
...
- 可视化：KnowledgeVisualizer（为仪表盘提供数据）
- 入口集成：NecoRAG 统一入口类，负责初始化与启动调度器
- 抽象基类：BaseKnowledgeUpdater、BaseMetricsCalculator 等，确保实现一致性
...
V["KnowledgeVisualizer<br/>可视化接口"]
...
- KnowledgeVisualizer：为 Dashboard 提供可视化所需的数据格式，包括健康度仪表盘、增长曲线、热力图等。
...
- 抽象基类：BaseKnowledgeUpdater、BaseMetricsCalculator 等，确保实现一致性与可替换性。
...
- [config.py:16-222](file://src/knowledge_evolution/config.py#L16-L222)
- [models.py:14-367](file://src/knowledge_evolution/models.py#L14-L367)
- [base.py:729-795](file://src/core/base.py#L729-L795)

## 架构总览
调度管理器采用“调度器 + 执行器 + 监控器 + 可视化”的分层架构：
...
- 可视化：KnowledgeVisualizer 提供仪表盘所需的数据格式，包括健康度、增长趋势、维度雷达等。
- 配置中心：KnowledgeEvolutionConfig 提供统一配置，支持默认、积极、保守与最小配置策略。
- 统一入口：NecoRAG 初始化时创建并启动调度器，自动注册默认任务。
...
- KnowledgeVisualizer 依赖指标计算器与更新器，负责可视化数据准备。
...
- [visualizer.py:29-47](file://src/knowledge_evolution/visualizer.py#L29-L47)
- [necorag.py:135-172](file://src/necorag.py#L135-L172)
...
- v3.3.0-alpha 版本中，调度器仍使用内置 threading 模块，生产环境建议集成 APScheduler 或 Celery。

章节来源
- [scheduler.py:321-381](file://src/knowledge_evolution/scheduler.py#L321-L381)
- [scheduler.py:325-327](file://src/knowledge_evolution/scheduler.py#L325-L327)
- [metrics.py:65-134](file://src/knowledge_evolution/metrics.py#L65-L134)
...
- 可视化数据缺失：检查指标计算器与更新器状态；确认数据缓存与历史记录配置。
...
### v3.3.0-alpha 版本特性与优化
- 版本标识：仓库版本文件明确标注 v3.3.0-alpha。
- 调度器内核稳定：保持了稳定的调度器实现，支持基本的定时任务与手动触发。
- 扩展点保留：为后续引入 APScheduler/Celery 等专业调度框架预留了适配器与便捷函数。

章节来源
- [VERSION_README.md:90-91](file://VERSION_README.md#L90-L91)
- [scheduler.py:557-688](file://src/knowledge_evolution/scheduler.py#L557-L688)